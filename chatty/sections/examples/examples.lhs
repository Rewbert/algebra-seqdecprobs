...
%
Imagining the goal is to reach state |five|, and that we had a reward function that rewarded us if the control we used took us closer to the goal.
%
\begin{code}
  goal : Nat
  goal = 5

  1d-reward : (x : 1d-state) -> 1d-control x ->
              1d-state -> Nat
  1d-reward x0 y x1 = -- reward function
\end{code}
%
